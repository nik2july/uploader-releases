...
describe('the raw batches offered to an editor', () => {
  test('a job with one link offers that single package', () => {
    const batches = rawBatches(job({ rawDataLink: 'https://drive.google.com/drive/folders/123456789' }));
    assert.equal(batches.length, 1);
    assert.equal(batches[0].label, 'Raw footage');
    assert.equal(batches[0].cloud, 'Google Drive');
  });

  test('two transfers in two clouds are both offered, oldest first', () => {
    const batches = rawBatches(job({
      rawDataLink: 'https://drive.google.com/drive/folders/123456789',
      desktopTransfers: {
        t2: { id: 't2', link: 'https://www.dropbox.com/scl/fo/xyz', purpose: 'raw', createdAt: '2026-09-14', bytes: 310, fileCount: 486 },
        t1: { id: 't1', link: 'https://drive.google.com/drive/folders/123456789', purpose: 'raw', createdAt: '2026-09-12', bytes: 842, fileCount: 1204 },
      },
    }));
    assert.deepEqual(batches.map(b => [b.label, b.cloud]), [
      ['Batch 1', 'Google Drive'],
      ['Batch 2', 'Dropbox'],
    ]);
  ...
  test('the headline link is not offered twice when a transfer already covers it', () => {
    const batches = rawBatches(job({
      rawDataLink: 'https://drive.google.com/drive/folders/123456789/',
      desktopTransfers: { t1: { id: 't1', link: 'https://drive.google.com/drive/folders/123456789', purpose: 'raw', createdAt: '2026-09-12', bytes: 842, fileCount: 1204 } },
    }));
    assert.equal(batches.length, 1);
  ...
  test('a link pasted in by hand is still offered alongside recorded transfers', () => {
    const batches = rawBatches(job({
      ...
      desktopTransfers: { t1: { id: 't1', link: 'https://drive.google.com/drive/folders/123456789', purpose: 'raw', createdAt: '2026-09-12', bytes: 1, fileCount: 1 } },
    }));
    assert.deepEqual(batches.map(b => b.cloud), ['Google Drive', 'Dropbox']);
  });
  ...
  test('batch folders are distinct and safe to create', () => {
    const batches = rawBatches(job({
      desktopTransfers: {
        t1: { id: 't1', link: 'https://drive.google.com/drive/folders/123456789', purpose: 'raw', createdAt: '2026-09-12' },
        t2: { id: 't2', link: 'https://www.dropbox.com/scl/fo/xyz', purpose: 'raw', createdAt: '2026-09-14' },
      },
    }));
    const names = batches.map(batchFolder);
    assert.deepEqual(names, ['Batch 1 - Google Drive', 'Batch 2 - Dropbox']);
