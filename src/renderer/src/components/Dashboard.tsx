...
import { Cloud, CloudOff, Film, Link2, Settings, Upload, Users } from 'lucide-react';
...
export function Dashboard(): React.JSX.Element {
  ...
  const completed = useMemo(() => transfers.filter(job => job.status === 'completed'), [transfers]);
  ...
  return (
    <div className="app-shell">
      <nav className="sidebar">
        ...
        <button className="nav-item" aria-current={view === 'completed'} onClick={() => go('completed')}>
          <Link2 size={16} /> Completed &amp; links
      ...
      <main className="main-area">
        ...
        {open ? (
          ...
          <WorkScreen kind={view} transfers={transfers} drive={drive} onScanStarted={opened} onSettings={() => go('settings')} />
        ) : view === 'completed' ? (
          <CompletedScreen transfers={completed} refresh={refresh} onOpen={setOpenId} />
        ) : (
